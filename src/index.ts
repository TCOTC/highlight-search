...
export default class PluginHighlight extends Plugin {
    ...
    onload() {
        this.host = new SearchHostImpl(this.eventBus, this.drag);
    ...
    addSearchElement(isFromTopBar: boolean = false) {
        this.host.cleanupInvalidComponents();
        ...
        Array.from(edits).forEach((edit) => {
            ...
            if (!existingElement) {
                ...
                this.host.register(element, instance);
                return;
            }

            const instance = this.host.get(existingElement);
            if (!instance) return;
